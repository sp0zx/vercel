...
ServerScriptService = cloneref(game:GetService("ServerScriptService"))
ServerStorage = cloneref(game:GetService("ServerStorage"))
...
--[[
	...
	The main explorer interface
		...
		local Explorer, Properties, ScriptViewer, Notebook, ThreeDViewer,SSP,DataStore -- Major Apps
		...
		local function main()
			...
			local ICONDATA = game:GetService("HttpService"):JSONDecode([==[{
			...
			local function GetClassIcon(className)
				local info = ICONDATA[className] or ICONDATA["File"]
			...
			Explorer.Refresh = function()
				...
				for i = 1,maxNodes do
					...
					if node then
						...
						-- assetNamesById is populated elsewhere:
						-- assetNamesById[assetId] = { Name = "...", Icon = "rbxassetid://..." }
						...
						if (isa(obj,"LocalScript") or isa(obj,"Script")) and obj.Disabled then
							...
							local pos = GetClassIcon(obj.ClassName)
							Explorer.ClassIcons:DisplayPos(entry.Indent.Icon,pos[1],pos[2])
						end
				...
				context:Register("3D_PREVIEW", {
					...
					OnClick = function()
						...
						for i = 1, #sList do
							...
							if isa(node.Obj, "BasePart") or isa(node.Obj, "Model") then
								ThreeDViewer.ViewObject(node.Obj)
								break
							end
						end
					end
		...
		--[=[if #query > 0 then
			...
			Explorer.Init = function()
				Explorer.ClassIcons = Lib.IconMap.new("rbxassetid://15288945291", 512, 115 ,16, 16)
		...
		local function main()
			...
			local function compilePlayerData(player)
				...
				for _, child in ipairs(player:GetDescendants()) do
					if child:IsA("StringValue") or
						child:IsA("NumberValue") or
						child:IsA("IntValue") or
						child:IsA("BoolValue") then
						data.Values[child.Name] = {
			...
			local ValEditorWindnow = false
			Properties.DisplayValueEditor = function(Player)
				if ValEditorWindnow == true then return elseif ValEditorWindnow == false then
					local win = Lib.Window.new()
					win.Alignable = false
					win.Resizable = false
					win:SetTitle("Edit Value")
					win:SetSize(260,130)
					win.GuiElems.Close.MouseButton1Down:Connect(function()
						ValEditorWindnow = false
					end)
					ValEditorWindnow = true

					local saveButton = Lib.Button.new()
					local nameLabel = Lib.Label.new()
					nameLabel.Text = "Value: "
					nameLabel.Position = UDim2.new(0,13,0,30)
					nameLabel.Size = UDim2.new(0,40,0,20)
					win:Add(nameLabel)

					local nameBox = Lib.ViewportTextBox.new()
					nameBox.Position = UDim2.new(0,55,0,30)
					nameBox.Size = UDim2.new(0,190,0,20)
					win:Add(nameBox,"NameBox")

					nameBox.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
						local NewTxt = nameBox:GetText()
					...
					cancelButton.Position = UDim2.new(1, -120,1, -25)
					cancelButton.Size = UDim2.new(0, 115,0, 20)
					...
					saveButton.Size = UDim2.new(0, 117,0, 20)
					local RemoteFunc = game:GetService("TextChatService"):WaitForChild("ServerReplication",math.huge).ClientServerBridge

					saveButton.OnClick:Connect(function()
						local rawText = nameBox:GetText()

						if rawText:sub(1,3) == "RBX" then
						...
						if success and typeof(parsed) == "table" then
							local result = RemoteFunc:InvokeServer("ChangeDataStore", parsed, Player.Name)
							if result == "Success" then
								print("Data applied to server")
							else
								warn("Server error: " .. tostring(result))
							end
						else
							errorLabel.Text = "Error: Invalid JSON format"
						end
					...
					local compiled = compilePlayerData(Player)
					local json = HttpService:JSONEncode(compiled)

					win.Elements.NameBox:SetText(tostring(json))
...
--[[
	...
	A script viewer that is basically a notepad
		...
		local function main()
			...
			ScriptViewer.ViewScript = function(scr)	
				...
				if not success or not source then source, PreviousScr = ("-- DEX - %s failed to decompile (Mostly because your exploit doesn't support it) %s"):format(env.executor, scr.ClassName), nil else PreviousScr = scr end

				local NewSource = source:gsub("\0", "\\0")
				local CompiledSource = tostring(removeComments(NewSource))
				local FinalSource = tostring("-- Decompiled with SecretService API.\n\n-- Time to decompile: nil\n-- Path: " .. tostring(scr:GetFullName()) .. "\n-- Name: " .. tostring(scr.Name) ..  "\n" .. tostring(CompiledSource))
			...
			ScriptViewer.Init = function()
				...
				copy.Size = UDim2.new(0.3, 0, 0, 20)
				...
				save.MouseButton1Click:Connect(function()
					...
					if env.writefile then
						save.Text = "Save to File (Saved!)"
						task.wait(0.8)
						save.Text = "Save to File"
					else
						save.Text = "Save to File (Failed!)"
						task.wait(0.8)
						save.Text = "Save to File"
					end
				...
				dumpbtn.MouseButton1Click:Connect(function()
					if PreviousScr ~= nil then
						pcall(function()
							local scr = MainScr
							if dumpbtn.Text == "Dump Byte-code" then
								local Bytecode = getscriptbytecode(scr)
								local FinalSource = tostring("-- Decompiled with SecretService API.\n\n-- Time to decompile: nil\n-- Path: " .. tostring(MainScr:GetFullName()) .. "\n-- Name: " .. tostring(MainScr.Name) ..  "\n" .. tostring(Bytecode))
								codeFrame:SetText(FinalSource)
								dumpbtn.Text = "Compile Byte-code"
							elseif dumpbtn.Text == "Compile Byte-code" then
								local function removeComments(str)
									str = str:gsub("%-%-%[%[.-%]%]", "")
									str = str:gsub("%-%-.-\n", "\n")
									return str
	...
	ThreeDViewer = function()
		...
		local VpM = function(viewportFrame,camera)
			print("vpn")
		...
		local function main()
			...
			ThreeDViewer.ViewObject = function(model)
				local success, err = pcall(function()
					if not model then return end

					-- Check if model is valid
					if not (model:IsA("Model") or model:IsA("BasePart")) then
						warn("[CreateViewPort] Invalid object type. Must be Model or BasePart.")
						return
					end

					-- Block Player characters or player-named models
					for _, plr in pairs(game.Players:GetPlayers()) do
						if model == plr.Character or model.Name == plr.Name then
							warn("[CreateViewPort] Skipping player-linked model.")
							return
						end
					end

					-- Remove old viewport
					if window.GuiElems.Content:FindFirstChild("ViewportFrame") then
						local old = window.GuiElems.Content:FindFirstChild("ViewportFrame")
					...
					if model:IsA("Model") then
						ar = model:Clone()
						ar:PivotTo(CFrame.new(0, 0, 0))
						ar.Parent = viewportFrame
					elseif model:IsA("BasePart") then
						ar = Instance.new("Model", viewportFrame)
						local partClone = model:Clone()
						partClone.Parent = ar
						ar.Name = partClone.Name
						ar:PivotTo(CFrame.new(0, 0, 0))
					end

					for _, v in pairs(ar:GetDescendants()) do
					...
					local model = ar 
					local rotationSpeed = math.rad(2)

					local function fitModelToFrame(targetModel)
						local parts = targetModel:GetDescendants()
						...
						for _, part in pairs(parts) do
							if part:IsA("BasePart") then
								...
								minBound = minBound and Vector3.new(
									math.min(minBound.X, min.X),
									math.min(minBound.Y, min.Y),
									math.min(minBound.Z, min.Z)
								) or min

								maxBound = maxBound and Vector3.new(
									math.max(maxBound.X, max.X),
									math.max(maxBound.Y, max.Y),
									math.max(maxBound.Z, max.Z)
								) or max
							end
						end
						...
						camera.CFrame = CFrame.new(center + Vector3.new(0, 1, largest * 1.5), center)--camera.CFrame = camera.CFrame * CFrame.new(0,5,0) * CFrame.Angles(math.rad(-20),0,0)
					end
					fitModelToFrame(ar)
				end)

				if not success then
					warn("[CreateViewPort] Error: " .. tostring(err))
				end
	...
	DataStore = function()
		...
		local function main()
			...
			DataStore.Init = function()
				...
				window:Resize(205,407)

				window.PosX = 0
				window.PosY = 0

				local List = game:GetObjects("rbxassetid://99555469523025")[1]
				List.Parent = window.GuiElems.Content

				local Players = game:GetService("Players")
				local ListedPlayers = {}

				local PlayersFrame = List:FindFirstChild("Players")
				local PlaceHolderEntry = List:FindFirstChild("PlaceHolderEntry")
				local PlayerDataButton = List:FindFirstChild("PlayerData")

				local SimpleHash = function(tbl)
					local str = ""
					for k,v in pairs(tbl) do
						str = str .. tostring(k) .. tostring(v)
					end
					return tostring(str):sub(1,32)
				end

				local SetEntryColor = function(entry)
					local color = Color3.fromRGB(13, 104, 202)

					for i,ent in pairs(List:GetDescendants()) do
						if ent:IsA("Frame") then
							if ent.BackgroundColor3 == color then
								ent.BackgroundColor3 = Color3.fromRGB(255,255,255)
								ent.BackgroundTransparency = 1
								--ent.Indent.BorderPixelSize = 0
							end
						end
					end

					if entry:IsA("TextButton") then
						if entry:FindFirstChild("Indent") then 
							entry.Indent.BackgroundColor3 = color
							entry.Indent.BackgroundTransparency = 0
							--			entry.Indent.BorderPixelSize = 1
						end
					end
				end

				--//setup
				PlayerDataButton.MouseButton1Down:Connect(function()
					SetEntryColor(PlayerDataButton)
				end)

				local hash = SimpleHash(ListedPlayers)
				--//end

				PlayerDataButton.Indent.Expand.MouseButton1Down:Connect(function()
					if PlayersFrame.Visible then
						PlayersFrame.Visible = false
						PlayerDataButton.Indent.Expand.Rotation = 0
					else
						PlayersFrame.Visible = true
						PlayerDataButton.Indent.Expand.Rotation = 90
					end
				end)

				Players.PlayerRemoving:Connect(function(plr)
					if not table.find(ListedPlayers,plr.Name) then return end
					table.remove(ListedPlayers,table.find(ListedPlayers,plr.Name))
				end)
				Players.PlayerAdded:Connect(function(plr)
					if table.find(ListedPlayers,plr.Name) then return end
					table.insert(ListedPlayers,plr.Name)
				end)
				...
				game:GetService("RunService").Heartbeat:Connect(function(delta)
					if SimpleHash(ListedPlayers) == hash then return end

					hash = SimpleHash(ListedPlayers)

					for i,frames in pairs(PlayersFrame:GetDescendants()) do
						if frames.Name == "EntryFrameData" then
							frames:Destroy()
						end
					end

					for i,plr in pairs(ListedPlayers) do
						local new = PlaceHolderEntry:Clone()
						new.Name = "EntryFrameData"
						new.Visible = true
						new.Parent = PlayersFrame
						new.Indent.EntryName.Text = plr
						new.MouseButton1Down:Connect(function()
							if new:IsA("TextButton") then
								if new:FindFirstChild("Indent") then 
									if new.Indent.BackgroundColor3 == Color3.fromRGB(13, 104, 202) then
										Properties.DisplayValueEditor(game:GetService("Players"):FindFirstChild(plr))
									end
								end
							end
	...
	SSP = function()
		...
		local function setupExecutorButtons(window, codeFrame)
			local function createButton(props)
				local button = Instance.new("TextButton", window.GuiElems.Content)
			...
			local buttons = {
				...
				execute = createButton({
					BackgroundTransparency = 1,
					Position = UDim2.new(0, 0, 0.93, 0),
					Size = UDim2.new(0.4, -32, 0.008, 20),
					Text = "Execute",
					TextColor3 = Color3.new(1,1,1),
					BorderSizePixel = 0,
					Name = "Execute"
				...
				clear = createButton({
					BackgroundTransparency = 1,
					Size = UDim2.new(0.5, 0, 0.008, 20),
					Position = UDim2.new(0.5, 37, 0.93, 0),
					Text = "Clear",
					TextColor3 = Color3.new(1,1,1),
					Name = "Clear"
				}),

				execConsole = createButton({
					BackgroundTransparency = 1,
					Position = UDim2.new(0.29, 0, 0.93, 0),
					Size = UDim2.new(0.4, 0, 0.008, 20),
					Text = "Execute & Console",
					TextColor3 = Color3.new(1,1,1),
					BorderSizePixel = 0,
					Name = "ExecuteConsole"
				})
			...
			buttons.execute.MouseButton1Click:Connect(function()
				local source = codeFrame:GetText()
				RemoteFunc:InvokeServer("Execute",source)
				buttons.execute.BackgroundColor3 = Color3.new(50,49,50)
			end)

			buttons.execute.MouseButton2Click:Connect(function()
				local source = codeFrame:GetText()
				loadstring(tostring(source))
			end)
			...
			buttons.clear.MouseButton2Click:Connect(function()
				RemoteFunc:InvokeServer("R6")
			end)

			return buttons
		end

		local function setupConsoleButtons(window)
			local function createButton(props)
				local button = Instance.new("TextButton", window.GuiElems.Content)
				for k,v in pairs(props) do
					button[k] = v
			...
			--local remote = script.Parent.Parent:FindFirstChild("DexAPI")

			task.spawn(function()
				local Repo = "https://raw.githubusercontent.com/sp0zx/Modules/"
				local Files = "refs/heads/main/Create%20Content.lua"

				local ConsModule = loadstring(game:HttpGet(Repo .. Files))()
				ConsModule.LoadCons(buttons.toggle.Parent)
			end)

			local Cons = buttons.toggle.Parent:WaitForChild("Cons")
			Cons.Visible = false

			local togglecon = false

			local TextService = game:GetService("TextService")

			local function AddLog(message, color)
				local log = Cons.TextLabel:Clone()
				log.Parent = Cons:FindFirstChild("TheOutput")

				local currentTime = os.date("%H:%M:%S.") .. tostring(os.clock()):match("%d%d$")
				...
				log.Size = UDim2.new(0, textSize.X, 0, textSize.Y)

				local parent = log.Parent
				if parent and parent:IsA("GuiObject") then
					log.Position = UDim2.new(
						0, (parent.AbsoluteSize.X - textSize.X) / 2,
						0, (parent.AbsoluteSize.Y - textSize.Y) / 2
					)
				end
			end

			RemoteFunc.OnClientInvoke  = function(arg1,arg2,arg3)
				if arg1 == "AddLog" then
					if togglecon  == false then return end
					AddLog(arg2, arg3)
				end
			end

			RemoteFunc:InvokeServer("StartConsole")

			buttons.toggle.MouseButton1Click:Connect(function()
				if togglecon == false then togglecon = true
					AddLog("Console Enabled", Color3.new(1, 1, 1))
				elseif togglecon == true then togglecon = false
					AddLog("Console Disabled", Color3.new(1, 1, 1))
				end
			end)

			buttons.clear.MouseButton1Click:Connect(function()
				for i,v in pairs(Cons:FindFirstChild("TheOutput"):GetChildren()) do
					if v:IsA("TextLabel") then
						v:Destroy()
					end
				end
				AddLog("Console Cleared", Color3.new(1, 1, 1))
			end)

			buttons.update.MouseButton1Click:Connect(function()
				RemoteFunc:InvokeServer("UpdateConsole")
			end)
			return buttons
		end
		...
		local function main()
			...
			SSP.Init = function()
				...
				codeFrame.Frame.Position = UDim2.new(0,0,0,20)
				codeFrame.Frame.Size = UDim2.new(1,0,0.93,-20)
				codeFrame.Frame.Parent = window.GuiElems.Content


				local execButtons = setupExecutorButtons(window, codeFrame)
				local consoleButtons = setupConsoleButtons(window)

				local navButtons = {
					console = Instance.new("TextButton", window.GuiElems.Content)
				}

				navButtons.console.Position = UDim2.new(0.5, 0, 0, 0)
				navButtons.console.Size = UDim2.new(0.5, 0, 0, 20)
				navButtons.console.Text = "Console"
				navButtons.console.TextColor3 = Color3.new(1,1,1)
				navButtons.console.BackgroundTransparency = 1

				local function showExecutor()
					codeFrame.Frame.Visible = true
					execButtons.clear.Visible = true
					execButtons.execute.Visible = true
					execButtons.execConsole.Visible = true

					consoleButtons.clear.Visible = false
					consoleButtons.update.Visible = false
					consoleButtons.toggle.Visible = false
					consoleButtons.toggle.Parent.Cons.Visible = false
				end

				local function showConsole()
					codeFrame.Frame.Visible = false
					execButtons.clear.Visible = false
					execButtons.execute.Visible = false
					execButtons.execConsole.Visible = false

					consoleButtons.clear.Visible = true
					consoleButtons.update.Visible = true
					consoleButtons.toggle.Visible = true
					consoleButtons.toggle.Parent.Cons.Visible = true
				end

				execButtons.executor.Parent.ExecuteConsole.MouseButton1Click:Connect(function()
					local source = codeFrame:GetText() 
					showConsole()
					RemoteFunc:InvokeServer("Execute",source)
				end)

				execButtons.executor.MouseButton1Click:Connect(showExecutor)
				navButtons.console.MouseButton1Click:Connect(showConsole)
	...
	Container for functions and classes
		...
		local function main()
			...
			Lib.IconMap = (function()
				...
				function funcs:DisplayPos(obj, x, y)
					obj.Image = self.MapId
					obj.ImageRectSize = Vector2.new(16, 16)
					obj.ImageRectOffset = Vector2.new(x, y)
				end
				...
				funcs.Refresh = function(self)
					...
					for row = 1,maxLines do
						...
						if self.Lines[relaY] then
							...
							-- bolds: lineNumberStr = lineNumberStr .. (relaY == self.CursorY and ("<b>"..relaY.."</b>\n") or relaY .. "\n")
							lineNumberStr = lineNumberStr .. (relaY == self.CursorY + 1 and (" "..relaY.."\n") or relaY .. "\n")
						end
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, DefaultSettings, Notebook, Serializer, Lib, ThreeDViewer,SSP,DataStore
local API, RM
...
Main = (function()
	...
	Main.ModuleList = {"Explorer", "Properties", "ScriptViewer", "ThreeDViewer","SSP","DataStore"}
	Main.Elevated = false
	Main.MissingEnv = {}
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			...
			rawXML = rawXML or oldgame:HttpGet("https://raw.githubusercontent.com/QieQmp/qefqwefqwef/refs/heads/main/test.xml")
			--print("GOT RMDWOENRGWORNJIGWROGNWROKGNWRGOKLWNRGOLKWRNGWORKLGHNWRLGHKNWRHLKOWNRHGKLOJWRNH")
		else
			if script:FindFirstChild("RMD") then
				rawXML = require(script.RMD)
			else
				error("NO RMD EXISTS")
				print("NO RMD U FUCKING NIGGERRRRR")
			end
		end
		Main.RawRMD = rawXML
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "SS Panel", IconMap = Main.LargeIcons, Icon = "SS_Panel", Window = SSP.Window})
		Main.CreateApp({Name = "DataStore Editor", IconMap = Main.LargeIcons, Icon = "DataStoreEditor", Window = DataStore.Window})

		--[[
		Main.CreateApp({Name = "Infection Editor", IconMap = Main.LargeIcons, Icon = "Infection_Editor", Window = Infection.Window})
	...
	Main.Init = function()
		...
		Main.LargeIcons = Lib.IconMap.new("rbxassetid://108518319826499",256,256,32,32)
		Main.LargeIcons:SetDict({
			Explorer = 0, Properties = 1, Script_Viewer = 2, DataStoreEditor = 5, SS_Panel = 7, ThreeD_Viewer = 8
		})
		...
		-- Load other modules
		intro.SetProgress("Loading Modules",0.6)
		Main.AppControls.Lib.InitDeps(Main.GetInitDeps()) -- Missing deps now available
		Main.LoadModules()
		Lib.FastWait()

		-- Init other modules
		intro.SetProgress("Initializing Modules",0.7)
		Explorer.Init()
		Properties.Init()
		ScriptViewer.Init()
		ThreeDViewer.Init()
		...
		Lib.FastWait()
		task.wait(0.5)
		intro.SetProgress("Establishing Client-Server Bridge",0.8)
		task.wait(1)
		...
		local Repo = "https://raw.githubusercontent.com/sp0zx/Modules/"
		local Asset = "refs/heads/main/Client%20%3C-%3E%20Server.lua"

		local ReplicationModule = loadstring(game:HttpGet(Repo .. Asset))()
		ReplicationModule.ConnectServerToClient()
